...
(function (_$) {
    ...
    window.tender = function () {
        ...
        // 匹配筛选可提交字段
        this.fields = [
            'modeltype', 'checkField', 'tyid', 'toid', 'gid', 'ptag', 'rand',
            'phone', 'rsastr', 'rsastatus', 'rsadata', 'zxxq', 'address', 'qq', 'zbgate', 'yuyue_type', 'square',
            ...
            'fitstate','source','verify','id','pos','oldaid','title','demotype','validCode','markCode', 'autoPop','xgtqbj','houses',
            'a_price','zzcost','method','shuidian'];
    };
    ...
    tender.prototype = {
        ...
        sendZbRequest: function(sendData,func) {
            ...
            if (!_$.md5) {
                    _$.getScript('http://static.to8to.com/gb_js/jQuery.md5.js?v=20170511', function () {
...
// 在回调函数之前存中转库id，供智齿客服回传数据使用
function setTmpYid(res) {
  // 存智齿客服所需要的用户信息
  if (res.tmpYid) {
    tender_setXgtCookie('tmpYid',res.tmpYid,1000*24*3600*1000)
  }
}
// 存加密后的手机号,供智齿客服回传数据使用
function setRsadata(rsadata) {
  tender_setXgtCookie("encryptNumber",rsadata,1000*24*3600*1000)
}
